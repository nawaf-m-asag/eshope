...
class PublicProductController
{
    ...
    public function getProducts(Request $request, GetProductService $productService, BaseHttpResponse $response)
    {
        ...
        if ($query) {
            ...
            SeoHelper::setTitle(__('Search result for ":query"', compact('query')))
                ->setDescription(__('Search result for ":query"', compact('query')));
        ...
        if ($request->ajax()) {
            ...
            return $response
                ->setData(view(Theme::getThemeNamespace('views.ecommerce.includes.product-items'), compact('products'))->render())
                ->setMessage($message);
        }
    ...
    public function getProduct($slug, Request $request)
    {
        ...
        return Theme::scope('ecommerce.product', compact('product', 'selectedAttrs', 'productImages'), 'plugins/ecommerce::themes.product')
            ->render();
    }
     ...
     */
    public function getProductTag($slug, Request $request, ProductTagInterface $tagRepository, BaseHttpResponse $response)
    {
        $slug = $this->slugRepository->getFirstBy([
            'key'            => $slug,
    ...
    public function getProductVariation($id, Request $request, BaseHttpResponse $response)
    {
        ...
        if ($product->with_storehouse_management && $product->quantity) {
            if ($product->quantity != 1) {
                $message = __(':number products available', ['number' => $product->quantity]);
            } else {
                $message = __(':number product available', ['number' => $product->quantity]);
            }
        }
        ...
        return $response
            ->setData([
                ...
                'price'                      => $product->price_with_taxes,
                'sale_price'                 => $product->front_sale_price_with_taxes,
                'original_price'             => $product->original_price,
                'image_with_sizes'           => $product->image_with_sizes,
                'display_price'              => format_price($product->price_with_taxes),
                'display_sale_price'         => format_price($product->front_sale_price_with_taxes),
     ...
     * @param GetProductService $getProductService
     * @param BaseHttpResponse $response
     * @return BaseHttpResponse|\Illuminate\Http\RedirectResponse|\Illuminate\Http\Response|Response
     */
