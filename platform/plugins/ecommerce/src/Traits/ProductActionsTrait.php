...
trait ProductActionsTrait
{
    ...
    public function postSaveAllVersions(
        ...
        foreach ($versionInRequest as $variationId => $version) {
            ...
            if (!$variation->product_id || $isUpdateProduct) {
                ...
                $productRelatedToVariation->images = json_encode(array_values(array_filter(Arr::get($version, 'images') ? Arr::get($version, 'images') : [])));
    ...
    public function getVersionForm(
        ...
        $productAttributes = $productAttributeRepository->getAllWithSelected($productId);
